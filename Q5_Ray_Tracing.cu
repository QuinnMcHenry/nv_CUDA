// This is a simple ray tracer that shoots rays top down toward randomly
// generates spheres and draws the sphere in a random color based on where
// the ray hits it.

#include "FreeImage.h"
#include "stdio.h"

#define DIM 2048
#define rnd(x) (x * rand() / RAND_MAX)
#define INF 2e10f

struct Sphere {
    float   r,b,g;
    float   radius;
    float   x,y,z;
    // Tells us if a ray hits the sphere; return the
    // depth of the hit, or -infinity if the ray misses the sphere
    __device__ float hit( float ox, float oy, float *n ) //__device so it can be used on kernel 
    {
        float dx = ox - x;
        float dy = oy - y;
        if (dx*dx + dy*dy < radius*radius)
        {
            float dz = sqrtf( radius*radius - dx*dx - dy*dy );
            *n = dz / sqrtf( radius * radius );
            return dz + z;
        }
        return -INF;
    }
};

#define SPHERES 80

/**
Each thread gets one pixel and calculates collisions with hit() on each sphere.
Other than that is the same as serial version, now there is no need for nested loop

**/
__global__ void drawSpheres(Sphere* spheres, char* red, char* green, char* blue)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x; // pixel index 
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x>=DIM || y>= DIM) return; 
    float ox = (x - DIM /2.0f);
    float oy = (y - DIM /2.0f);
    float r=0, g=0, b=0;
    float maxZ = -INF;

    for (int i = 0; i < SPHERES; i++)
    {
        float n;
        float t = spheres[i].hit(ox, oy, &n);
        if (t > maxZ)
        {
            float fscale = n;
            r = spheres[i].r * fscale;
            g = spheres[i].g * fscale;
            b = spheres[i].b * fscale;
            maxZ = t;
        }
    }
    int offset = x + y * DIM;
    red[offset] = (char) (r * 255);
    green[offset] = (char) (g * 255);
    blue[offset] = (char) (b * 255);
}


int main()
{
  FreeImage_Initialise();
  atexit(FreeImage_DeInitialise);
  FIBITMAP * bitmap = FreeImage_Allocate(DIM, DIM, 24);
  srand(time(NULL));

  char *red;
  char *green;
  char *blue;

  // Dynamically create enough memory for DIM * DIM array of char.
  // By making these dynamic rather than auto (e.g. char red[DIM][DIM])
  // we can make them much bigger since they are allocated off the heap
  red = (char *) malloc(DIM*DIM*sizeof(char));
  green = (char *) malloc(DIM*DIM*sizeof(char));
  blue = (char *) malloc(DIM*DIM*sizeof(char));

  // Create random spheres at different coordinates, colors, radius
  Sphere spheres[SPHERES];
  for (int i = 0; i<SPHERES; i++)
  {
        spheres[i].r = rnd( 1.0f );
        spheres[i].g = rnd( 1.0f );
        spheres[i].b = rnd( 1.0f );
        spheres[i].x = rnd( (float) DIM ) - (DIM/2.0);
        spheres[i].y = rnd( (float) DIM ) - (DIM/2.0);
        spheres[i].z = rnd( (float) DIM ) - (DIM/2.0);
        spheres[i].radius = rnd( 200.0f ) + 40;
  }
  
  Sphere* dev_spheres;
  char* dev_red;
  char* dev_green;
  char* dev_blue;
  
  cudaMalloc((void**)&dev_spheres, SPHERES*sizeof(Sphere));  

  cudaMalloc((void**)&dev_red, DIM*DIM*sizeof(char));
  cudaMalloc((void**)&dev_green, DIM*DIM*sizeof(char));
  cudaMalloc((void**)&dev_blue, DIM*DIM*sizeof(char));
  
  cudaMemcpy(dev_spheres, spheres, SPHERES*sizeof(Sphere), cudaMemcpyHostToDevice);

  // do not need to memcpy rgb from host to dev because they are empty anyway
  
  dim3 block(32,32);
  dim3 grid(DIM / block.x, DIM / block.y);
  drawSpheres<<<grid, block>>>(dev_spheres, dev_red, dev_green, dev_blue); // 4096 blocks, 1024 threads in each 
  
  cudaMemcpy(spheres, dev_spheres, SPHERES*sizeof(Sphere), cudaMemcpyDeviceToHost); 
  cudaMemcpy(red, dev_red, DIM*DIM*sizeof(char), cudaMemcpyDeviceToHost);
  cudaMemcpy(green, dev_green, DIM*DIM*sizeof(char), cudaMemcpyDeviceToHost);
  cudaMemcpy(blue, dev_blue, DIM*DIM*sizeof(char), cudaMemcpyDeviceToHost);
  cudaFree(dev_spheres);
  cudaFree(dev_red);
  cudaFree(dev_green);
  cudaFree(dev_blue);

  RGBQUAD color;
  for (int i = 0; i < DIM; i++)
  {
    for (int j = 0; j < DIM; j++)
    {
      int index = j*DIM + i;
      color.rgbRed = red[index];
      color.rgbGreen = green[index];
      color.rgbBlue = blue[index];
      FreeImage_SetPixelColor(bitmap, i, j, &color);
    }
  }
	
  FreeImage_Save(FIF_PNG, bitmap, "ray.png", 0);
  FreeImage_Unload(bitmap);
  free(red);
  free(green);
  free(blue);
/**
it is obvious when running both executables there is significant speedup - simple 'time' shows ~3.7 second for serial version
and ~1.2 for the parallelized version. On the cpu, we have a nested loop iterating over a 2048x2048 matrix, and other than
that the only code is the init for freeImage, mem allocations, and random generation of the sphere structs. (and saving the
png.) These are not parallelizable, and becasue of the magnitude of the loop, a conservative assessment is that the program
 spends about 95% of its execution time in said loop. This particular loop is "embarassingly parallel" because none
of the pixels are dependent on one another, that is, it does not matter what the status of adjacent pixels are to the curr-
-ent one. Also, the GPU completely eliminated the need for the loops at all, so I will call the GPU
speedup as 100%.

So, assuming p = 0.95, Amdahl's law says that speedup (S) = 1 / (1 - 0.95) + 0.95 / 100

..Which is 1 / 0.05 + 0.0095, which is about a 16.8x speedup. It is probably the case that the program 
spends more than 95% of the time in that loop so speedup could be between 16x and 50x.
**/
  return 0;
}


